...
```text
python3 "4 Tooling/Folder.py" strip-prefix ROOT --match REGEX
python3 "4 Tooling/Folder.py" strip-prefix ROOT --match REGEX --apply
```
...
```text
python3 "4 Tooling/Folder.py" strip-prefix "4 Tooling" \
  --files-only \
  --include "*.py" \
  --match '^[0-9]+ '
```
