---
uid: YT5Y7F
form: '<a href="../3%20Standard%20Forms/11%20%F0%9F%93%96%20Index.md" uid="BZJASV">documentation-system:§3.11</a>'
description: >-
  `Consult when` *the Documentation System needs compilation, address resolution,
  or harness projection* `to` **select the tool that performs the required
  control operation**.
---

# Tooling

This location contains executable tools whose module docstrings are themselves
indexed documentation.

<!-- BEGIN index -->
<!-- This block was created by running `4 Tooling/1 🛠️ Navigation Crawler.py`; run it whenever indexed information or classification may have changed. -->

- <a href="1%20%F0%9F%9B%A0%EF%B8%8F%20Navigation%20Crawler.py" uid="45E225">documentation-system:§4.1</a> — 🛠️ Navigation Crawler
  - `Read in full and follow when` *a Documentation System corpus may have changed or an address must be resolved* `to` **validate derived control state, refresh index blocks and controlled links, or resolve the requested address without hand-maintaining indexes**.
- <a href="2%20%F0%9F%9B%A0%EF%B8%8F%20Harness%20Installer.py" uid="N5986D">documentation-system:§4.2</a> — 🛠️ Harness Installer
  - `Read in full and follow when` *a repository has installed the Documentation System but an agent harness cannot discover it under the skill name declared by SKILL.md* `to` **project the canonical Documentation System directory into each selected harness skill directory with an idempotent symlink**.
<!-- END index -->
