...
| `.project` | Controlled project sideband for repository-local Work Management state and retained Records that share the same address-opaque retrieval boundary. | Descend and control metadata-bearing Work Management and Record artifacts, including UID minting, controlled Form links, and research-provenance validation beneath `.project/Records/Research/`; descendants have no Documentation System address and do not enter generated indexes. Preserve the sideband and descendant names from Documentation System folder normalization. | Read active Work Management locations when repository work is being captured, planned, executed, resumed, handed off, or closed. Read `.project/Records/` only when the user explicitly requests retained record material or current work requires the relevant evidence. Do not load cold Archive or unrelated Records merely because they exist. |
...
Reserved controlled sidebands (`.decisions` and `.project`) are
