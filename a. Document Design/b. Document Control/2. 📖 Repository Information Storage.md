...
<a href="1.%20%F0%9F%93%96%20Folder%20Conventions.md" uid="TRJS8V">documentation-system:§a.b.1</a>.
In particular, retained research beneath `.project/Records/Research/` is controlled through the
`.project` sideband: metadata-bearing artifacts retain UID identity and compiler
validation while remaining outside
normal address and index navigation. `.project` applies the same address-opaque
control pattern to mutable and retained Work Management information while also
keeping that state out of this repository's dot-prefixed published projection.
...
with current authority during normal agent retrieval, the
`.project/Records/Research/` collector supplies that policy while preserving
ordinary human file navigation.
...
Document Forms under Document Design Assemblies.
...
human-browsable. Repository-local Work Management therefore uses ordinary
Markdown files and directories beneath the reserved `.project/` sideband when
the repository itself owns that tracking. The sideband is selected for concrete
control benefits: one deterministic work-state root, explicit separation from
current authority, address-opaque retrieval, and exclusion from this
repository's dot-prefixed published projection.

Work Management owns the internal `.project/` responsibility structure,
lifecycle, and Work Object semantics. Document Control owns the sideband's
corpus behavior. Another external system or Git storage plane becomes
appropriate when collaboration, assignment, notification, independent
lifecycle, or another concrete process requirement selects it.
...
<a href="../../b.%20Editing%20Concepts/3.%20%F0%9F%93%96%20Validation%20Evidence.md" uid="5SK5Z4">documentation-system:§b.3</a>.
