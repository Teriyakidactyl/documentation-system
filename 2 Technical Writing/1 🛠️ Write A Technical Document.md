...
description: >-
  ...
  before drafting prose that satisfies them while checking each representation
  against the commitments that govern it**.
---
...
Use validation reasoning throughout authoring. As each representation becomes
inspectable, compare it with the committed description, quadrant, outline,
writing style, applicable Document Form or Document Elements, repository
conventions, and task-specific accepted criteria that govern it. When those
authorities determine a correction, make it as part of authoring rather than
deferring every check to a separate post-writing stage.

A bounded Validation pass is a separate assurance operation, not the default
final step of authoring. Apply
<a href="../7%20Editing/2%20%F0%9F%9B%A0%EF%B8%8F%20Validate%20an%20Artifact.md" uid="BCSYYG">documentation-system:§7.2</a>
only when a user, artifact, workflow, acceptance gate, independent-review
requirement, or other governing process explicitly selects that assurance.
