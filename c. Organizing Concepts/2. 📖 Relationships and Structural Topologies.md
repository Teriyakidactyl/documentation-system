...
A **collector folder** is a containment node whose children share a retrieval,
control, lifecycle, or operational boundary without sharing semantic ownership.
Use one when independently owned units need one predictable place to converge
for access or retention.

The collector owns only the common containment rule. Each child keeps the
meaning, Form, lifecycle semantics, and authority of its own kind. Therefore a
collector must not imply that heterogeneous children become one conceptual type
merely because they are siblings.

A collector is appropriate when all of these conditions hold:

- the children are already established structural units;
- one cross-cutting boundary genuinely applies to every collected child;
- callers benefit from one deterministic entry point;
- the collector can state its invariant without redefining child semantics.

Do not create a collector merely to shorten a tree or hide unresolved
classification. If the proposed parent needs to reinterpret what its children
are, return to structural-unit or ownership reasoning instead.
